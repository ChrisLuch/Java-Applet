...
parser.add_option("-v", dest="doVersion", help="Dumps the version of the program and exits", action="store_true")
...
def showUsageMessage():
	print("Usage: %s file.jar [--deps file1.jar[:files.jar]]" % sys.argv[0])
...
def appendAndCleanJS(baseClassName, jsLinesSet, jsOutput):
	for l in open(baseClassName+".js", encoding="utf-8"):
		if l not in jsLinesSet:
			jsLinesSet.add(l);
			jsOutput.write(l);
	if os.path.isfile(baseClassName+"_llvm.js"):
		for l in open(baseClassName+"_llvm.js", encoding="utf-8").readlines():
			if l not in jsLinesSet:
				jsLinesSet.add(l);
				jsOutput.write(l);
	# Delete JS
...
def compileClassFiles(cheerpj, cheerpjClassPath, tempDir, jarFile, jarPath, jsOutput, splitFiles):
	...
	def do_compile(f):
		...
		p = subprocess.Popen([cheerpj, "-llvm", "-llvm-exceptions", "-cp", cheerpjClassPath, f], cwd=jarUnpackPath,stdout=PIPE, stderr=STDOUT);
		out,_ = p.communicate();
		ret = p.returncode
		if ret != 0:
		...
		p = subprocess.Popen([cheerpPath+"/bin/llc","-march=cheerp","-cheerp-reserved-names=a,b,f,i,p","-cheerp-no-boilerplate","-cheerp-registerize-legacy","%s_llvm.opted.bc" % baseClassName,"-o","%s_llvm.js" % baseClassName], cwd=jarUnpackPath, stdout=PIPE, stderr=STDOUT);
		out,_ = p.communicate()
		ret = p.returncode;
		if ret != 0:
			print("Failure compiling %s" % f)
		...
		if option.stubNatives:
			...
			out,_ = p.communicate();
		return f, True
	def do_compile_with_fallback(f):
		if not f.endswith(".class"):
			return f, False;
		if f in BAD_CLASSES:
			return f, False;
		f, compiled = do_compile(f);
		if not compiled:
			# Try again with the legacy codegen
			print("Using legacy codegen for %s" % f);
			p = subprocess.Popen([cheerpj, "-cp", cheerpjClassPath, f], cwd=jarUnpackPath,stdout=PIPE, stderr=STDOUT);
			out,_ = p.communicate();
			ret = p.returncode
			return f, ret == 0
		return f, True
	for f, compiled in executor.map(do_compile_with_fallback, jarFile.namelist()):
		if not compiled:
		...
		if not splitFiles:
			...
			if os.path.isfile(llvmName):
				llvmSize = os.path.getsize(llvmName)
				jsSize += llvmSize;
			packageMap[packageName].size += jsSize;
	...
	for p in packagesAndSizes:
		...
		if parentPackage != "" and packageMap[parentPackage].size <= 1024*1024:
			...
			if packageMap[p.path]:
				...
				jsPackage = open(jarPath + "." + p.path.replace('/','.') + ".js", "w", encoding="utf-8");
...
def runOnDir(tempDir):
	...
	if option.depsPaths:
		for usedJarPath in option.depsPaths.split(":"):
			try:
				...
				print("No such file:", err.filename)
				showUsageMessage()
				exit(1)
			unpackPath = unpackClassFiles(tempDir, usedJarFile, usedJarPath);
	...
	for jarPath in jarsToCompile:
		...
		jsOutput = open(jarPath + ".js", "w", encoding="utf-8");
